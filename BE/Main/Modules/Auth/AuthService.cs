...
public class AuthService : IAuthService
{
    ...
    public async Task<string> Login(AuthenticationDTO loginCredentials)
    {
        try
        {
            ...
            UserModel? user = await _authContext.Set<UserModel>()
    ...
    public async Task<UserModel> Register(RegistrationDTO registration)
    {
        try
        {
            ...
            UserModel? userCheck = await _authContext.Set<UserModel>()
            ...
            await _authContext.Set<UserModel>().AddAsync(newUser);
    ...
    public async Task<string> RecoverAccount(string email)
    {
        try
        {
            ...
            List<UserModel> usersWithThisEmail = await _authContext.Set<UserModel>().Where(p => p.Email == email).ToListAsync();
            ...
            await _authContext.SaveChangesAsync(); // Save any changes, if applicable.
    ...
    public async Task<UserModel?> GetCurrentUser() =>
        ...
        ? await _authContext.Set<UserModel>().SingleOrDefaultAsync(u =>
    ...
    public async Task<IEnumerable<string>?> GetUserTags(string? specific = ".", Guid? userId = null)
    {
        try
        {
            ...
            return await _authContext.Set<UserModel>()
                .Where(u => u.Id == userId)
                .Select(p => p.Tags)
    ...
    public async Task ChangePassword(string email, string username, string currentPassword, string newPassword)
    {
        try
        {
            ...
            var user = await _authContext.Set<UserModel>()
            ...
            {
                From = { new MailboxAddress(_config.GetSection("SmtpSettings")["SenderName"], _config.GetSection("SmtpSettings")["SenderEmail"]) },
    ...
    public async Task<List<string>> CheckUserRights(IEnumerable<string> rightsToCeck, Guid? userId = null)
    {
        ...
        {
            userId = userId ?? (await GetCurrentUser())?.Id;
            UserModel? user = await _authContext.Set<UserModel>().Where(p => p.Id == userId).SingleOrDefaultAsync();
    ...
    {
        UserModel? user = await _authContext.Set<UserModel>().FindAsync(userId ?? (await GetCurrentUser())?.Id);
