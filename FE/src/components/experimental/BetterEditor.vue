  ...
  <div class="flex-column gap-2 align-items-center justify-content-center">
    <div class="pages-container">
      <div class="page">
        <div
          id="editableContent"
          class="contenteditable border-1 custom-shadow-1 gorder-round border-gray-200"
          style="height: 90vh; aspect-ratio: 1/1.414"
...
const pages = defineModel('pages', {
  type: Array<{ content: string; customData: string }>,
  default: { content: '', customData: '' }
})

const seePageNumber = defineModel('seePageNumber', {
  type: Number,
  default: 1
})
...
function checkOverflow() {
  ...
  if (editableElement && pageHTML && editableElement.scrollHeight > pageHTML.clientHeight)
    createNewPage()
}
...
const validatePageNumber = (event: InputNumberInputEvent) => {
  ...
  event.value = isNaN(value) || value < 1 || value > (pages.value?.length ?? 0 > 0) ? 1 : value
}
</script>
