...
<template>
  <div
    id="sidebar"
    class="fixed bg-primary z-5 h-screen custom-shadow-1 flex-column"
    :class="{ 'border-round-right': sidebarHover }"
    style="overflow: auto !important"
    @mouseenter="sidebarHover = true"
    @mouseleave="sidebarHover = false"
  >
    <div class="flex-column py-3 h-full overflow-auto">
      <div
    ...
    <div v-else class="w-full p-2">
      ...
      />
      <Button icon="pi pi-cog" size="small" @click="onNodeClick('/settings')" />
    </div>
  </div>
  <Button
    ...
    text
  />
  <router-view v-slot="{ Component }" class="sm:ml-7 h-screen">
